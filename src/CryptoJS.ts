module OpenAPI {
  ...
  export class CryptoJS {
    ...
    static AES(data: string, key: string, iv: string): any {
      // 密钥
      const secretKey = OpenAPI_CryptoJS.enc.Utf8.parse(key)

      // iv
      const randomIv = OpenAPI_CryptoJS.enc.Utf8.parse(iv)

      // 加密CBC
      const encryptData = OpenAPI_CryptoJS.AES.encrypt(data, secretKey, {
    ...
    static AES_DECRYPT(data: string, key: string, iv: string): any {
      ...
      // 加密CBC
      const encryptData = OpenAPI_CryptoJS.AES.decrypt(data, secretKey, {
        mode: OpenAPI_CryptoJS.mode.CBC,
        iv: randomIv,
        padding: OpenAPI_CryptoJS.pad.Pkcs7,
      })
