var SceneUtils = (function () {
    ...
    SceneUtils.prototype.getGridDynamicObsStatus = function (gridP, excepter, gridSceneObjects, checker) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (checker) {
                var sign = checker.collisionGroup + "_" + so.collisionGroup;
    ...
    SceneUtils.prototype.collsionCheckGrid = function (checker, posP, posGridP, trendP, calcTrendP) {
        ...
        if (posGridP.x == checker.posGrid.x && posGridP.y == checker.posGrid.y)
            return res;
        var gridSceneObjects = null;
        if (WorldData.moveToGridCenter) {
            gridSceneObjects = this.gridSceneObjects[posGridP.x][posGridP.y].concat();
            for (var i = 0; i < gridSceneObjects.length; i++) {
                var targetSo = gridSceneObjects[i];
        ...
        var dynamicObsState = this.getGridDynamicObsStatus(posGridP, checker, gridSceneObjects, checker);
                    ...
                    if (jRect && jRect.width > 0 && jRect.height > 0) {
                        ...
                        if (!hasBridge) {
                            if (tSo.bridge)
                                ...
                                var collisionSign = checker.collisionGroup + "_" + tSo.collisionGroup;
